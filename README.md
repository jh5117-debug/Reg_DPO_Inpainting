# DiffuEraser Finetune

> 合作者完整操作指南 — 从零开始搭建 DiffuEraser Finetune 训练环境

📄 原始文件：[YTBV_Finetune.txt](YTBV_Finetune.txt)

---

## 📖 操作指南

本指南包含两种 Finetune 模式（二选一）：
- **A) Non-Prompt 版** — 训练时使用固定文本 `"clean background"`
- **B) Prompt 版** — 训练时使用 VLM 预生成的真实场景描述

两种模式共享相同的数据集和权重，仅训练脚本不同。

### HuggingFace 仓库

| # | 仓库 | 内容 |
|---|------|------|
| 1 | `JiaHuang01/DiffuEraser-finetune-code` | 基础代码 + 数据集 |
| 2 | `JiaHuang01/DiffuEraser-finetune-prompt-code` | Prompt 版代码 + Captions |
| 3 | `JiaHuang01/DiffuEraser-finetune-weights` | 模型权重 |

---

### 第一步：前置环境

```bash
pip install -U huggingface_hub
huggingface-cli login
```

> ⚠️ **重要**：你需要一个 HuggingFace Token（在 https://huggingface.co/settings/tokens 创建）。该 Token 将在第七步中填入 `run_finetune_all.sbatch` 文件。

---

### 第二步：下载所有文件（一次性全部下载）

```bash
mkdir -p ~/DiffuEraser_downloads
cd ~/DiffuEraser_downloads

echo "===== [1/3] 下载基础代码 + 数据集 ====="
huggingface-cli download JiaHuang01/DiffuEraser-finetune-code \
    --repo-type dataset \
    --local-dir ./finetune-code

echo "===== [2/3] 下载 Prompt 版代码 + Captions ====="
huggingface-cli download JiaHuang01/DiffuEraser-finetune-prompt-code \
    --repo-type dataset \
    --local-dir ./finetune-prompt-code

echo "===== [3/3] 下载模型权重 ====="
huggingface-cli download JiaHuang01/DiffuEraser-finetune-weights \
    --repo-type dataset \
    --local-dir ./finetune-weights

echo "===== 下载完成！====="
```

---

### 第三步：搭建 Non-Prompt 版本 (Project A)

```bash
mkdir -p ~/DiffuEraser_finetune
cd ~/DiffuEraser_finetune

cp ~/DiffuEraser_downloads/finetune-code/code_base.tar.gz .
cp ~/DiffuEraser_downloads/finetune-code/DAVIS.tar .
cp ~/DiffuEraser_downloads/finetune-code/YTBV.tar .
cp ~/DiffuEraser_downloads/finetune-code/setup_project.sh .

mkdir -p weights
cp -r ~/DiffuEraser_downloads/finetune-weights/* weights/

bash setup_project.sh
```

看到 `"🎉 Setup Complete!"` 说明 Non-Prompt 版搭建成功。

---

### 第四步：搭建 Prompt 版本 (Project B)

Prompt 版通过**符号链接**复用 Non-Prompt 版的数据集和权重，不会重复占用 ~60GB 的磁盘空间。

#### 4.1 创建目录、解压代码

```bash
mkdir -p ~/DiffuEraser_finetune_prompt
cd ~/DiffuEraser_finetune_prompt

tar -xzf ~/DiffuEraser_downloads/finetune-code/code_base.tar.gz
tar -xzf ~/DiffuEraser_downloads/finetune-prompt-code/code_base_prompt.tar.gz
tar -xzf ~/DiffuEraser_downloads/finetune-prompt-code/captions.tar.gz
```

> ⚠️ 注意：必须先解压 `code_base.tar.gz`，再解压 `code_base_prompt.tar.gz`。顺序不能反！Prompt 版脚本需要覆盖 Non-Prompt 版同名文件。

#### 4.2 创建符号链接（复用数据集 + 权重）

```bash
ln -sf ~/DiffuEraser_finetune/dataset/DAVIS  dataset/DAVIS
ln -sf ~/DiffuEraser_finetune/dataset/YTBV   dataset/YTBV
ln -sf ~/DiffuEraser_finetune/weights         weights
```

#### 4.3 验证 Prompt 版结构

```bash
ls -la dataset/DAVIS dataset/YTBV weights captions/all_captions_merged.yaml
```

如果所有 4 项都能正常显示（不报错），说明 Prompt 版搭建成功。

---

### 第五步：安装 Python 依赖（两个版本共用同一环境）

```bash
cd ~/DiffuEraser_finetune
conda env create -f environment.yml
conda activate diffueraser
```

---

### 第六步：配置 Accelerate（多卡训练）

```bash
accelerate config
```

推荐配置：
- This machine / multi-GPU
- num_processes: 你的 GPU 数量（如 8）
- mixed_precision: bf16

---

### 第七步：修改脚本路径（一键替换）

脚本中硬编码了原作者的路径，需要替换成你的路径。根据你要运行的版本，执行对应的替换命令。

#### 7-A) Non-Prompt 版路径替换

```bash
cd ~/DiffuEraser_finetune
PROJECT_DIR=$(pwd)
WEIGHTS_DIR="${PROJECT_DIR}/weights"

for f in finetune_stage1.sh finetune_stage2.sh run_finetune_all.sbatch \
         save_checkpoint_stage1.py save_checkpoint_stage2.py \
         train_DiffuEraser_stage1.py train_DiffuEraser_stage2.py; do
    sed -i "s|/home/hj/Train_Diffueraser|${PROJECT_DIR}|g" $f
    sed -i "s|/home/hj/DiffuEraser_new/weights|${WEIGHTS_DIR}|g" $f
    echo "Fixed: $f"
done

# 填入你的 HuggingFace Token
sed -i 's|hf_xxxxxxxxxxxxxxxxx|你的HF_Token|g' run_finetune_all.sbatch
```

#### 7-B) Prompt 版路径替换

```bash
cd ~/DiffuEraser_finetune_prompt
PROJECT_DIR=$(pwd)
WEIGHTS_DIR="${PROJECT_DIR}/weights"

for f in finetune_stage1.sh finetune_stage2.sh run_finetune_all.sbatch \
         save_checkpoint_stage1.py save_checkpoint_stage2.py \
         train_DiffuEraser_stage1.py train_DiffuEraser_stage2.py; do
    sed -i "s|/home/hj/Train_Diffueraser_prompt|${PROJECT_DIR}|g" $f
    sed -i "s|/home/hj/Train_Diffueraser/dataset|${PROJECT_DIR}/dataset|g" $f
    sed -i "s|/home/hj/DiffuEraser_new/weights|${WEIGHTS_DIR}|g" $f
    echo "Fixed: $f"
done

# 填入你的 HuggingFace Token
sed -i 's|hf_xxxxxxxxxxxxxxxxx|你的HF_Token|g' run_finetune_all.sbatch
```

---

### 第八步：开始训练（选择你要运行的版本）

> ⚠️ `run_finetune_all.sbatch` 中默认 `--num_processes 8`，如果你的 GPU 数量不同，请手动修改 sbatch 文件。

#### 8-A) 运行 Non-Prompt 版

```bash
cd ~/DiffuEraser_finetune
mkdir -p logs converted_weights
sbatch run_finetune_all.sbatch
```

查看日志：`tail -f logs/DiffuEraser_Total-*.out`

#### 8-B) 运行 Prompt 版

```bash
cd ~/DiffuEraser_finetune_prompt
mkdir -p logs converted_weights
sbatch run_finetune_all.sbatch
```

查看日志：`tail -f logs/DiffuEraser_Prompt-*.out`

---

## 📋 附录

<details>
<summary>附录 A：完成后的目录结构</summary>

```
~/
├── DiffuEraser_downloads/              ← 下载缓存（可删除）
│   ├── finetune-code/
│   ├── finetune-prompt-code/
│   └── finetune-weights/
│
├── DiffuEraser_finetune/               ← Non-Prompt 版
│   ├── train_DiffuEraser_stage1.py
│   ├── train_DiffuEraser_stage2.py
│   ├── finetune_stage1.sh
│   ├── finetune_stage2.sh
│   ├── run_finetune_all.sbatch
│   ├── save_checkpoint_stage1.py
│   ├── save_checkpoint_stage2.py
│   ├── environment.yml
│   ├── libs/
│   ├── diffueraser/
│   ├── dataset/
│   │   ├── DAVIS/                      ← 真实数据 (~844 MB)
│   │   ├── YTBV/                       ← 真实数据 (~5.26 GB)
│   │   ├── finetune_dataset.py
│   │   └── utils.py ...
│   └── weights/
│       ├── stable-diffusion-v1-5/
│       ├── diffuEraser/
│       ├── sd-vae-ft-mse/
│       └── animatediff-motion-adapter-v1-5-2/
│
└── DiffuEraser_finetune_prompt/        ← Prompt 版
    ├── train_DiffuEraser_stage1.py     ← Prompt 版 (使用 caption)
    ├── train_DiffuEraser_stage2.py     ← Prompt 版 (使用 caption)
    ├── finetune_stage1.sh
    ├── finetune_stage2.sh
    ├── run_finetune_all.sbatch
    ├── save_checkpoint_stage1.py
    ├── save_checkpoint_stage2.py
    ├── libs/
    ├── diffueraser/
    ├── dataset/
    │   ├── DAVIS → 符号链接
    │   ├── YTBV  → 符号链接
    │   ├── finetune_dataset_caption.py ← ✨ Prompt 版 Dataset
    │   └── utils.py ...
    ├── captions/
    │   ├── all_captions_merged.yaml    ← 训练用 (3561 条)
    │   └── *.yaml
    └── weights → 符号链接
```

</details>

<details>
<summary>附录 B：两种版本的核心区别</summary>

| 项目 | Non-Prompt 版 | Prompt 版 |
|------|--------------|-----------|
| 训练 Caption | 硬编码 `"clean background"` | VLM 生成的真实场景描述 |
| Dataset 类 | `FinetuneDataset` | `FinetuneDatasetWithCaption` |
| 新增训练参数 | 无 | `--caption_yaml` |
| Caption 数据 | 无 | `captions/all_captions_merged.yaml` |
| Caption 数量 | 0 | 3561 (90 DAVIS + 3471 YTVOS) |
| 权重 / 数据集 | 相同 | 相同 (符号链接) |

</details>

<details>
<summary>附录 C：常见问题</summary>

**Q1: CUDA OOM 怎么办？**
在 sbatch/sh 中添加或确认 `--gradient_checkpointing`，或减小 `--nframes` / `--resolution`

**Q2: 如何查看训练是否使用了 Caption？**
查看训练启动日志，Prompt 版会打印 `"FinetuneDatasetWithCaption: total X videos, 3561 captions loaded"`，Non-Prompt 版会打印 `"FinetuneDataset: total X videos"`

**Q3: 两个版本可以同时运行吗？**
可以。它们在不同目录，互不影响。确保 GPU 数量足够即可。

**Q4: 下载缓存可以删除吗？**
搭建完两个版本后即可安全删除 `~/DiffuEraser_downloads`，节省磁盘空间。

**Q5: accelerate config 每个版本要单独配吗？**
不需要。accelerate config 是全局配置，两个版本共用同一份配置。

</details>
