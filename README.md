...
```
================================================================
     DiffuEraser Finetune 合作者完整操作指南 (统一版)
================================================================
...
Prompt 版通过 符号链接 复用 Non-Prompt 版的数据集和权重，
不会重复占用 ~60GB 的磁盘空间。
...
⚠️ 注意：必须先解压 code_base.tar.gz，再解压 code_base_prompt.tar.gz
   顺序不能反！Prompt 版脚本需要覆盖 Non-Prompt 版同名文件。
...
⚠️ 注意：run_finetune_all.sbatch 中默认 --num_processes 8，
   如果你的 GPU 数量不同，请手动修改 sbatch 文件。
...
└── DiffuEraser_finetune_prompt/        ← Prompt 版
    ...
    ├── save_checkpoint_stage1.py
    ├── save_checkpoint_stage2.py
    ├── generate_captions_ytvos.py
    ├── merge_captions.py
    ├── libs/
    ├── diffueraser/
    ├── propainter/
    ├── dataset/
    │   ├── DAVIS → 符号链接到 ../DiffuEraser_finetune/dataset/DAVIS
    │   ├── YTBV  → 符号链接到 ../DiffuEraser_finetune/dataset/YTBV
    │   ├── finetune_dataset.py         ← (不使用)
    │   ├── finetune_dataset_caption.py ← ✨ Prompt 版 Dataset
    │   └── utils.py ...
    ├── captions/
    │   ├── all_captions_merged.yaml    ← 训练用 (3561 条)
    │   ├── all_captions_BR.yaml
    │   ├── all_captions_ytvos.yaml
    │   └── *.yaml
    └── weights → 符号链接到 ../DiffuEraser_finetune/weights
...
Q1: CUDA OOM 怎么办？
A1: 在 sbatch/sh 中添加或确认 --gradient_checkpointing，
    或减小 --nframes / --resolution

Q2: 如何查看训练是否使用了 Caption？
A2: 查看训练启动日志，Prompt 版会打印
...
Q4: 下载缓存 ~/DiffuEraser_downloads 可以删除吗？
A4: 搭建完两个版本后即可安全删除，节省磁盘空间。

Q5: accelerate config 每个版本要单独配吗？
A5: 不需要。accelerate config 是全局配置 (~/.cache/huggingface/accelerate/)，
    两个版本共用同一份配置。
```
