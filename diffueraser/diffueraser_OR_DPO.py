...
def read_priori(priori, fps, n_total_frames, img_size):
    if not priori:  # Bypass path when priori is None or empty string
        return []
...
class DiffuEraser:
    ...
    def forward(self, validation_image, validation_mask, priori, output_path,
        ...
        if prioris: # Only slice prioris if it's not empty
        ...
        if prioris:
            ...
            latents_shape = (real_video_length, 4, tar_height//8, tar_width//8)
            latents = randn_tensor(latents_shape, device=torch.device(self.device), dtype=prompt_embeds_dtype, generator=generator)
            latents = latents * self.vae.config.scaling_factor
